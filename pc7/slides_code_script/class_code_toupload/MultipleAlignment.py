...
class MultipleAlignment():
        ...
        for i in range(len(charsCol)):
            for j in range(i+1, len(charsCol)):
                if charsCol[i] != "-" or charsCol[j] != "-":
                    if charsCol[i] == "-" or charsCol[j] == "-":
                        res += self.alignpars.g
                    else:
                        res += self.alignpars.sm.score_pair(charsCol[i], charsCol[j])
    ...
    def scoreSP (self, alignment):
        ...
        for i in range(len(alignment)):
            res += self.ScoreColumn(alignment.column(i))
...
def test_prot():
    ...
    print(alinm)
    print(alinm.column(0))
    print(ma.ScoreColumn(alinm.column(0)))
    print(alinm.column(2))
    print(ma.ScoreColumn(alinm.column(2)))
